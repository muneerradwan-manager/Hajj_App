import 'package:bawabatelhajj/core/cubits/safe_cubit.dart';
import 'package:bawabatelhajj/features/complaints/domain/usecases/get_cached_complaints_usecase.dart';
import 'package:bawabatelhajj/features/complaints/domain/usecases/get_complaints_usecase.dart';

import 'complaints_state.dart';

class ComplaintsCubit extends SafeCubit<ComplaintsState> {
  ComplaintsCubit(this._getComplaintsUseCase, this._getCachedComplaintsUseCase)
    : super(const ComplaintsState.initial());

  final GetComplaintsUseCase _getComplaintsUseCase;
  final GetCachedComplaintsUseCase _getCachedComplaintsUseCase;

  Future<void> loadComplaints() async {
    final cached = await _getCachedComplaintsUseCase();
    if (cached != null && cached.isNotEmpty) {
      safeEmit(state.copyWith(
        status: ComplaintsStatus.loaded,
        complaints: cached,
        isRefreshing: true,
      ));
    } else {
      safeEmit(state.copyWith(
        status: ComplaintsStatus.loading,
        isRefreshing: false,
      ));
    }

    final result = await _getComplaintsUseCase();

    result.fold(
      (failure) => safeEmit(
        state.copyWith(
          status: state.complaints.isEmpty
              ? ComplaintsStatus.error
              : ComplaintsStatus.loaded,
          errorMessage: failure.userMessage,
          isRefreshing: false,
        ),
      ),
      (complaints) => safeEmit(
        state.copyWith(
          status: ComplaintsStatus.loaded,
          complaints: complaints,
          isRefreshing: false,
        ),
      ),
    );
  }
}
